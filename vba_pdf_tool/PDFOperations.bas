Attribute VB_Name = "modPMOperations"
Option Explicit

'Exports the active sheet to PDF using the provided path
Public Sub PM_ExportActiveSheet(ByVal outputPath As String)
...
Public Sub PM_MergeSelectedPDFs(ByVal firstPDF As String)
...
Public Sub PM_SplitSelectedPDF(ByVal pdfPath As String)
    Dim pdDoc As Acrobat.AcroPDDoc
    Dim i As Long
    ...
    outputFolder = PM_GetFolder("Select folder for split pages")
...
Public Sub PM_DeletePagesFromSelectedPDF(ByVal pdfPath As String)
    Dim pdDoc As Acrobat.AcroPDDoc
...
Private Function PM_GetFolder(prompt As String) As String
    Dim fd As FileDialog
    Set fd = Application.FileDialog(msoFileDialogFolderPicker)
    fd.Title = prompt
    If fd.Show Then PM_GetFolder = fd.SelectedItems(1)
End Function
